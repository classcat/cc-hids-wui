...
echo '<div class="smaller">'.date("Y年m月d日 H時i分s秒").'</div><br />';
// echo '<div class="smaller2">'.date('F dS Y h:i:s A').'<br />';

if($USER_monitoring == 1)
{
    echo ' -- '.$ossec_refresh_time.' 秒毎に更新</div><br />';
...
<table><tr valign="top">
    ...
    <td>開始日時: &nbsp;<input type="text" name="initdate" 
    id="i_date_a" size="17"
    ...
    終了日時: &nbsp;<input type="text" name="finaldate" id="f_date_a" size="17"
...
echo '<tr><td>最低限レベル:</td><td><select name="level" class="formText">';
if($u_level == 1)
{
...
/* Category */
echo '</td><td>
     カテゴリ: </td><td><select name="grouppattern" class="formText">';
...
/* Str pattern */
echo '</td></tr><tr><td>    
    パターン: </td><td><input type="text" name="strpattern" size="16" 
                    value="'.$u_pattern.'" class="formText" /></td>';


/* Log formats */
echo '<td>
     ログ・フォーマット: </td><td><select name="logpattern" class="formText">';
...
/* Rule pattern */
echo '</td><td>
    ユーザ: </td><td><input type="text" name="userpattern" size="8" 
...
/* Rule pattern */
echo '</td><td>
    ルール ID: </td><td><input type="text" name="rulepattern" size="8" 
...
if($output_list == NULL || $output_list[1] == NULL)
{
    if($used_stored == 1)
    {
        echo "<b class='red'>検索結果はありません。(search expired). </b><br />\n";
        // echo "<b class='red'>Nothing returned (search expired). </b><br />\n";
    }
    else
    {
        echo "<b class='red'>検索結果はありません。</b><br />\n";
    }
    return(1);
}
...
echo "<b>検索されたアラート総計: </b>".$output_list[0]{'count'}."<br />";
// echo "<b>Total alerts found: </b>".$output_list[0]{'count'}."<br />";

if($output_list[0]{'pg'} > 1)
{
    echo "<b>出力は</b> " . $output_list[0]{'pg'} . " <b>ページに分割されています。</b><br />";
...
{
    echo "<b class='red'>検索結果はありません。(or search expired). </b><br />\n";
